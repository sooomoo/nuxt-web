...
// 此处的公钥与私钥都是32位
export const newBoxKeyPairFromArray = (
  pub: Uint8Array,
  pri: Uint8Array,
): KeyPair => {
...
// 此处的公钥与私钥都是32位
export const newSignKeyPairFromArray = (
  pub: Uint8Array,
  pri: Uint8Array,
): KeyPair => {
  return { publicKey: pub, privateKey: pri };
...
export const encodeSecureString = (
  signKey: Uint8Array,
  boxKey: Uint8Array,
): string => {
 ...
 * @returns
 */
export const getSecuretsFromCookie = (
  cookies: string[],
): Secrets | undefined => {
  const parsedCookies = parseCookies(cookies);
  const sessionId =
    parsedCookies.find((c) => c.name === import.meta.env.VITE_COOKIE_SK1_NAME)
      ?.value ?? "";
  const clientKey =
    parsedCookies.find((c) => c.name === import.meta.env.VITE_COOKIE_SK2_NAME)
      ?.value ?? "";
...
export const ensureSecurets = (): Secrets => {
  ...
  if (!pubKeys.box || !pubKeys.sign || !priKeys.box || !priKeys.sign) {
    ...
    sessionId.value = encodeSecureString(
      signKeyPair.publicKey,
      boxKeyPair.publicKey,
    );
    clientKey.value = encodeSecureString(

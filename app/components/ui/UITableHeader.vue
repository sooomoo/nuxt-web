...
const emit = defineEmits<{
    (e: "column-width-changed", col: T, delta: number): void;
}>();
...
const handleResize = (leftColIndex: number, rightColumnIndex: number, deltaX: number, _deltaY: number) => {
    ...
    col.width = newVal;
    emit("column-width-changed", col, deltaX);
};
</script>
...
<template>
    <div class="ui-flex ui-flex-align-stretch ui-table-header">
        <div v-for="(col, colIndex) in finalColumns" :key="col.field" class="column-item" :class="colClass"
            ...
            <UIResizer v-if="colIndex < finalColumns.length - 1" :class="finalResizerClass"
                :active-class="finalResizerActiveClass" :direction="'hor'" :style="{
                    width: '4px',
